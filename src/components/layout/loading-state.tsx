...
export function LoadingState({ text = "YawaTech" }: LoadingStateProps) {
  ...
  return (
    <div
      ref={preloaderRef}
      className={`fixed inset-0 z-50 flex items-center justify-center ${isLoading ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
    >
      <div
        ref={containerRef}
      ...
      >
        <div className="absolute z-10">
          ...
          <div
            ref={spinnerRef}
            className="mx-auto mb-14 h-36 w-36 rounded-full border-[10px] border-[#059669]/90 border-t-white"
          />

          <div
            className={`select-none text-center text-5xl tracking-wider md:text-8xl ${aeonik.className}`}
          >
            {characters.map((item, i) => (
              <span
                ...
                className="inline-block text-[#059669]"
        ...
        <div
          ref={sectionRightRef}
          className="fixed right-0 top-0 h-full w-1/2 bg-white"
